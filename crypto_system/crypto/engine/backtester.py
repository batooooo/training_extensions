...
class Backtester:
    ...
    def run(self, data: pd.DataFrame) -> BacktestResult:
        ...
        for ts, price in close.items():
            ...
            # --- entries (fractional sizing: crypto trades in fractions of a coin) ---
            equity = cash + qty * price
            if qty == 0 and sig is Signal.BUY and self.risk.check_daily_loss(equity):
                fill = price * (1 + self.slippage_pct)
                budget = min(self.risk.order_budget(equity), cash - self.commission)
                size = budget / fill if fill > 0 else 0.0
                if size > 0 and size * fill + self.commission <= cash + 1e-9:
