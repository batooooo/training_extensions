...
class RiskManager:
    ...
    def order_budget(self, equity: float) -> float:
        """Quote-currency amount to spend on one entry (for notional/fractional
        orders, e.g. crypto). = min(equity*max_position_pct, max_position_notional)."""
        return self._budget(equity)
